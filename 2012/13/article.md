...
development all it takes is a few instances of `git commit -a -m 'Good enough'`
...
and organizing programs and libraries. But while traditional development has
had around 40 years to mature, config management is fairly young in comparison
...
honest all the ideas espoused here are simply applications of the [unix
...
When writing modules like these, they should be, well, modular. They should do
one thing and do it well. For instance, a module for installing a web
application shouldn't try to also manage a database service, or configure
...
together to solve the entire problem. If people want to use your module and
also back up related files, they won't be forced to use your backup tool - they
can use your module to provide the service, and use their module to handle
backups.
...
that they expose, so that you can see what options you can tune and configure
without having to have complete mastery of the service that its managing. The
advantage of this is that you have a clean delineation between how the core
elements of the service work, and how you're implementing them.
...
system-specific configuration and presents you with a simple set of controls to
configure vhosts, apache modules, and ensure that the necessary packages are
installed and the service is running. For instance, when using this module you
...
end services like unicorn. You can use this module to run apache, and combine
it with other modules to build the rest of your configuration.
...
couple of twists. One is that since these modules cannot be reused elsewhere,
it can make sense to embed site specific data in templates and manifests.
Secondly, these modules are located in a different place on the filesystem.
Using the Puppet `modulepath` setting or chef `cookbook_path` setting, you can
specify a list of locations to check for modules. You can take advantage of
this to locate reusable base blocks in one place, and weird blocks in another
place.
...
It also can be used to control access - perhaps one team manages an internal
service only, so they can handle the configuration management for that service.
However this team won’t be administering the rest of your infrastructure.
Giving them access to the weird-blocks directory means they’ll be able to do
their job, but they’ll be bound to respecting the interfaces of the base-blocks
instead of taking shortcuts and putting site specific changes in your base
blocks.
...
only include other classes. That is, they exist almost entirely to aggregate
classes into usable units and configure their data. The following example would
be an example of everything you would need to bring up the mrepo infrastructure
on a node:
...
This final level takes all our implementations of apache and mysql and applies
them, controls system access, and provides for a complete redmine stack.
 Including this one class is all it takes to provide for every requirement of a
redmine instance, so deploying more machines for a specific role means
including a single self contained class. Instead of having to copy a long list
of requirements from one node to another, we portray this final level as class
providing for roles.
